...
def create_personalised_email(lead, sender):
    ...
    prompt = f"Help write a personalised email to {lead['Name']} and person's email address {lead['Email']}.\
                    Search about the {lead['Company']} in internet(new,blog etc) and check what all things they have undertaken\
                        in AI & ML field. Make use of the {lead['Context']} in email to show how sender knows the lead.Also see the {lead['LinkedIn']} profile of the person to understand the kind of post the \
...
def fetch_email_list_and_mail(leads_data):
    ...
    try:
        for lead in leads_data:
            ...
            st.write(f"Fetching Details for the lead: {lead['Name']}")
            st.write("\n")
            ...
            #send_email('smtp.gmail.com',587, sender_details["sender_email"], 'key value', sender_details["sender_email"], lead["Email"],'Enhance Your Data Journey with FOSFOR - A Game-Changer from LTIMindtree',email_content)
            lead["Email_status"]=f"Email sent on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        return leads_data
    except Exception as e:
        print("An error occurred: ", str(e))
